...
{
    // comment: Snell's law -> etai sin(theta1) = etat sin(theta2)
    // ior -> index of refraction
    float cosi = clamp(-1, 1, dotProduct(I, N));
    // etai is the refractive index of the incident medium (usually set to 1, that is, air)
    // etat is the refractive index of the transmission medium.
    ...
    float eta = etai / etat;
    // If k < 0, total reflection occurs(no refraction occurs).
...
void Renderer::Render(const Scene &scene)
{
    ...
    for (int j = 0; j < scene.height; ++j)
    {
        for (int i = 0; i < scene.width; ++i)
        {
            ...
            // 2. Map the normalized coordinates to [-1, 1] interval
            // 3. multiple scale (the dist of camera to screen is default 1) and AR
            // Reference: https://zhuanlan.zhihu.com/p/431092843
